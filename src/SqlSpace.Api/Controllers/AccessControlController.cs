    ...
    ICurrentUserService currentUserService) : ApiController
{
    ...
    [EndpointDescription("Parameters: connectionId (path) target connection id, request (body) target user email + access mode + optional restricted tables. Acting admin is resolved from the current authenticated user in JWT claims.")]
    ...
    [EndpointDescription("Parameters: connectionId (path) target connection id, targetUserId (path) user being updated, request (body) full-access flag + optional restricted tables. Acting admin is resolved from the current authenticated user in JWT claims.")]
    ...
    [EndpointDescription("Parameters: connectionId (path) target connection id, targetUserId (path) user to revoke. Acting admin is resolved from the current authenticated user in JWT claims.")]
    ...
    [EndpointSummary("Check if the current user has access to the connection")]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status401Unauthorized)]
    ...
    [EndpointSummary("Check if the current user can access a specific table -  null schema name only for mysql")]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiResponse<bool>), StatusCodes.Status401Unauthorized)]
    ...
    [EndpointSummary("Get all table names accessible by the current user")]
    [EndpointDescription("Parameters: connectionId (path) target connection id. User is resolved from current authenticated JWT claims.")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(userId))
        {
            return UnauthorizedResponse<ICollection<string>>();
        }
